...
local function GetPlayerBio(player)
    local char = player.Character
    if not char then return nil end
    local head = char:FindFirstChild("Head")
    if not head then return nil end
    for _, obj in ipairs(head:GetChildren()) do
        if obj:IsA("BillboardGui") and obj.Name ~= "SpectraTag" then
...
RunService.Heartbeat:Connect(function()
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        local bio = GetPlayerBio(player)
        if bio and bio ~= "" then
            local key = player.Name
            if LastBioSeen[key] ~= bio then
                LastBioSeen[key] = bio
                OnMessage(bio, player)
            end
        else
            LastBioSeen[player.Name] = nil
        end
    end
end)
...
if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        p.Chatted:Connect(function(msg) OnMessage(msg, p) end)
    end
